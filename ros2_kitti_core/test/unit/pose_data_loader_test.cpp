...
class TestPoseDataLoader : public r2k_core_test::TestWithPoseIO
{
public:
  using PoseDataLoader = r2k_core::PoseDataLoader;
...
TEST_F(TestPoseDataLoader, ReadFileFailure)
{
  ...
  const auto timestamps = r2k_core_test::generate_test_timestamps(1, 3);
  ASSERT_GT(timestamps.size(), std::size_t{0});

  r2k_core::PoseDataLoader loader(
...
TEST_F(TestPoseDataLoader, NormalOperations)
{
  ...
  const auto timestamps = r2k_core_test::generate_test_timestamps(1, number_readings);
  ASSERT_EQ(timestamps.size(), number_readings);

  const auto test_header = get_test_header();
  r2k_core::PoseDataLoader loader("test_pose_loader", test_header, std::string{kChildFrameFrameId});
  ...
  for (std::size_t i = 0; i < number_readings; i++) {
    ...
    const auto header_timestamp = r2k_core::Timestamp(pose_stamped.header.stamp);
